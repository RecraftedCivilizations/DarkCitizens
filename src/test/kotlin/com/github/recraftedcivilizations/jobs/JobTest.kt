...
internal class JobTest {
    ...
    private val job =
        Job("Foo", "Bar", 5, emptySet<ITask>(), emptySet(), 10, 10, 10, false, icon, Random.nextBoolean(), randomString(), dPlayerManager, jobManager, bukkitWrapper)
    ...
    fun join(){
        ...
        jobManager.createJob(job1.name, job1.group, job1.playerLimit, emptySet(), job1.canDemote, job1.baseIncome, job1.baseXPGain, job1.minLvl, false, job1.permissionRequired, job1.icon, job1.leaveOnDeath, prefix = "")
        jobManager.createJob(job2.name, job2.group, job2.playerLimit, emptySet(), job2.canDemote, job2.baseIncome, job2.baseXPGain, job2.minLvl, false, job2.permissionRequired, job2.icon, job2.leaveOnDeath, prefix = "")
    ...
    fun leave(){
        val jobManager = JobManager(dPlayerManager, lawManager)
        val job1 = createRandomJob(bukkitWrapper, icon, dPlayerManager, jobManager)
        val job2 = createRandomJob(bukkitWrapper, icon, dPlayerManager, jobManager)
        jobManager.createJob(job1.name, job1.group, job1.playerLimit, emptySet(), job1.canDemote, job1.baseIncome, job1.baseXPGain, job1.minLvl, false, job1.permissionRequired, icon, job1.leaveOnDeath, job1.prefix)
        jobManager.createJob(job2.name, job2.group, job2.playerLimit, emptySet(), job2.canDemote, job2.baseIncome, job2.baseXPGain, job2.minLvl, false, job2.permissionRequired, icon, job2.leaveOnDeath, job1.prefix)
